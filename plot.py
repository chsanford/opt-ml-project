...
# agd_NCE
for fname, filenames in all_filenames.items():
    logs = load_logs([filenames['gd'], filenames['agd_NCE']])
    fig, ax = plt.subplots()
    ax.margins(0)
    ax.set_xlabel('epochs')
    ax.set_ylabel('losses')
    ax.plot(np.mean(logs[0]['train_losses'], axis=0), label='GD (Train)', color='#1f77b4')
    ax.plot(np.mean(logs[1]['train_losses'], axis=0), label='AGD with NCE (Train)', color='#ff7f0e')
    if not fname == 'cnn':
        ax.plot(np.mean(logs[0]['test_losses'], axis=0), '--', label='GD (Test)', color='#1f77b4')
        ax.plot(np.mean(logs[1]['test_losses'], axis=0), '--', label='AGD with NCE (Test)', color='#ff7f0e')
    else:
        ax.set_ylim(top=4) 
    ax.legend()
    fig.savefig(fname+"_NCE.png", dpi=600)
