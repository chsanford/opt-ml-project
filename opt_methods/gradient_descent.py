...
class GradientDescent(Optimizer):
	...
	def __init__(self, is_ml, neural_net_params=None, learning_rate=0.1, noise_r=0.1, noise_T=10, noise_eps=0.1, momentum=0, NCE=False, NCE_s=0.1, NCE_gamma=2.5):
		self.is_ml = is_ml
		if is_ml:
	...
	def step(self, closure=None, is_verbose=True):
	    ...
	    if self.NCE:
	        ...
	        if is_verbose:
	            print("*NCE part* f(xt): %.3f, f(yt): %.3f, <grad_f(yt),xt-yt>: %.3f, ||xt-yt||^2: %.3f, ||vt||: %.3f" % (f_xt, f_yt, g_yt.dot((xt-yt)).item(), torch.norm((xt-yt), p=2).pow(2).item(), norm_vt.item()))
	        if norm_vt > 0 and f_xt <= f_yt + g_yt.dot((xt-yt)) - self.NCE_gamma / 2 * (torch.norm((xt-yt), p=2).pow(2)):
	...
	def step_not_ml(self, f, x, is_verbose=True):
		...
		if self.noise_r > 0:
		    if 'noise_count' not in state:
		        state['noise_count'] = 0
		    if is_verbose:
		        print("*Noise part* grad l2 norm: %.3f, count: %d" % (np.linalg.norm(f.grad(x), ord=2), state['noise_count']))
		...
		if self.NCE:
		    ...
		    if is_verbose:
		        print("*NCE part* f(xt): %.3f, f(yt): %.3f, <grad_f(yt),xt-yt>: %.3f, ||xt-yt||^2: %.3f, ||vt||: %.3f" % (f.eval(xt), f.eval(yt), np.dot(f.grad(yt), xt-yt), pow(np.linalg.norm((xt-yt), ord=2), 2), norm_vt))
		    if norm_vt > 0 and f.eval(xt) <= f.eval(yt) + np.dot(f.grad(yt), xt-yt) - self.NCE_gamma / 2 * pow(np.linalg.norm((xt-yt), ord=2), 2):
		        if norm_vt >= self.NCE_s:
		            x = xt
		            if is_verbose:
		                print("setting x_{t+1} = xt")
